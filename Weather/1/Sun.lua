...
function rscalc(when, where)
    ...
    local currentMinutes = tonumber(currentHour) * 60 + tonumber(currentMin)
	
	if currentMinutes >= setminutes or currentMinutes < riseminutes then
        SKIN:Bang('!SetVariable _DayOrNight "1')
    else
        SKIN:Bang('!SetVariable _DayOrNight "0')
    end
	-- SKIN:Bang('!SetVariable _DayOrNight "'..riseminutes..'"')
